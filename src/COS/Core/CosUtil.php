 ...
 * Class CosUtil
 *
 * Css工具类，主要供CosClient使用，用户也可以使用本类进行返回结果的格式化
 *
 * @package COS
 */
class CosUtil
{
    const COS_CONTENT = 'content';
    const COS_LENGTH = 'length';
    const COS_HEADERS = 'headers';
    const COS_MAX_OBJECT_GROUP_VALUE = 1000;
    const COS_MAX_PART_SIZE = 5368709120; // 5GB
    const COS_MID_PART_SIZE = 10485760; // 10MB
    const COS_MIN_PART_SIZE = 102400; // 100KB
    ...
    public static function validateOptions($options)
    {
        ...
        if ($options != NULL && !is_array($options)) {
            throw new CosException ($options . ':' . 'option must be array');
        }
    }
    ...
    {
        if (empty($content)) {
            throw new CosException("http body content is invalid");
        }
    }
     ...
     * @return void
     */
    public static function throwCosExceptionWithMessageIfEmpty($name, $errMsg)
    {
        if (empty($name)) {
            throw new CosException($errMsg);
        }
    }
...
BBB;
    ...
    {
        $content_md5 = "";
        if (($to_pos - $from_pos) > self::COS_MAX_PART_SIZE) {
            return $content_md5;
        }
    ...
    /**
     * 生成DeleteMultiObjects接口的json消息
     *
     * @param string[] $objects
     * @param bool $quiet
     * @return string
     */
    public static function createDeleteObjectsJsonBody($objects, $quiet)
    {
        $body = "{";
        $body = $body . '"Quiet"' . ":" . json_encode($quiet) . ",";
        $body = $body . '"Objects"'. ":" . '[';
        $cnt = 0;

        foreach ($objects as $obj) {
            $body = $body . '{' . '"Key"' . ':' . json_encode($obj). '}';
               if ($cnt+1 !== count($objects)) {
                $body = $body . ",";
                $cnt++;
            }
        }
    ...
    /**
     * 生成CompleteMultipartUpload接口的json消息
     *
     * @param array[] $listParts
     * @return string
     */
    public static function createCompleteMultipartUploadJsonBody($listParts)
    {
        $body = '{"Parts":[';
        $cnt = 0;
        foreach ($listParts as $node) {
            $body = $body . json_encode($node);
            if ($cnt+1 !== count($listParts)) {
                $body = $body . ",";
                $cnt++;
            }
        }
    ...
    public static function decodeKey($key, $encoding)
    {
        ...
        if ($encoding == "url") {
            ...
            throw new CosException("Unrecognized encoding type: " . $encoding);
        }
    }
}
